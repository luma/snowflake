module RedisGraph
  module Element
    module ClassMethods
      ...
      # @api public
      def get(key)
        node_attributes = RedisGraph.connection.hgetall( redis_key(key) )
      ...
      def rename(from_key, to_key)
        RedisGraph.connection.renamenx( redis_key(from_key), redis_key(to_key) )
      ...
      # @api public      
      def destroy!(key)
#        raise NotImplementedError, 'Modules that include RedisGraph::Element must implement a destroy! method.'
        # @todo error handling
        RedisGraph.connection.del( redis_key(key) )
